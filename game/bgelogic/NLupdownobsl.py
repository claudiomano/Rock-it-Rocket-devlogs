...
def _initialize(owner):
    ...
    PAR0002.game_object = "NLO:Empty.005"
    PAR0002.attribute_name = "worldPosition"
    network.add_cell(CON0001)
    network.add_cell(PAR0002)
    network.add_cell(ACT0000)
    owner["IGNLTree_up-down-obsl"] = network
    network._owner = owner
    network.setup()
    network.stopped = not owner.get('NL__up-down-obsl')
    return network

def pulse_network(controller):
    owner = controller.owner
    network = owner.get("IGNLTree_up-down-obsl")
    if network is None:
